...
| :--- | :--- | :--- | :--- |
| **1. Visual Validation** | Google Earth / Sentinel images | **Implemented**: GEE pipeline downloads optical & radar patches and crops central farm grids to avoid boundary mixing. | [gee_timeseries_pipeline.py](file:///Users/ananyalakshmi/PES/AgriTech/code/data_engineering/gee_timeseries_pipeline.py) |
| **2. Crop Pattern Validation** | NDVI time-series curve | **Implemented**: Growth stage engine correlates observed NDVI profiles with crop templates to detect misreporting. | [growth_stage_engine.py](file:///Users/ananyalakshmi/PES/AgriTech/code/crop_intelligence/growth_stage_engine.py) |
| **3. Weather Validation** | ERA5 rainfall & temp | **Implemented**: Ingestion of monthly aggregates from ERA5 Land collection to cross-check reported causes of loss. | [biological_validation.py](file:///Users/ananyalakshmi/PES/AgriTech/code/crop_intelligence/biological_validation.py) |
| **4. Stress Validation** | Simulated drought/flood | **Implemented**: Run experiments simulate monsoon drought stress during sensitive flowering stages using the Jensen model. | [run_experiments.py](file:///Users/ananyalakshmi/PES/AgriTech/code/crop_intelligence/run_experiments.py) |
| **5. Regional Validation** | Known crop-growing regions | **Implemented**: Empirical separability studies compare Presto embeddings across Punjab, Maharashtra, and Andhra Pradesh. | [regional_analysis.py](file:///Users/ananyalakshmi/PES/AgriTech/code/data_engineering/regional_analysis.py) |
| **6. Model Validation** | Train-test split on samples | **Implemented**: 80/20 train-test splits on 8,192 pixel sequences. Mamba benchmarked against LSTM and Transformers. | [run_experiments.py](file:///Users/ananyalakshmi/PES/AgriTech/code/temporal_intelligence/run_experiments.py) |
| **7. Literature Validation** | Compare with expected phenology | **Implemented**: Database contains stage sensitivity coefficients ($K_y$ and $K_t$) and stage durations from FAO papers. | [crop_knowledge_db.py](file:///Users/ananyalakshmi/PES/AgriTech/code/crop_intelligence/crop_knowledge_db.py) |
...
* **Implementation**: 
  ...
  - Implemented a `spatial_buffer` parameter of 16 in [gee_timeseries_pipeline.py](file:///Users/ananyalakshmi/PES/AgriTech/code/data_engineering/gee_timeseries_pipeline.py). It averages a central $32 \times 32$ window of the farm patch, neutralizing border contamination.
* **Verification Command**:
  ```bash
  python data_engineering/gee_timeseries_pipeline.py
  ```
...
* **Implementation**:
  - [growth_stage_engine.py](file:///Users/ananyalakshmi/PES/AgriTech/code/crop_intelligence/growth_stage_engine.py) computes the Pearson correlation coefficient between the observed NDVI curve and the expected crop templates.
...
* **Visual Output**: Saved in `figures/crop_stress_validation_report.png` (Panel A: Paddy Growth Stage & Phenology Comparison; Panel D: Crop Misreporting & Fraud Audit Template Match).
...
* **Implementation**:
  ...
  - [biological_validation.py](file:///Users/ananyalakshmi/PES/AgriTech/code/crop_intelligence/biological_validation.py) maps temperatures to thermal stress indices and computes soil moisture indices.
...
* **Implementation**:
  - Integrates the **Jensen Yield Loss Model** in [biological_validation.py](file:///Users/ananyalakshmi/PES/AgriTech/code/crop_intelligence/biological_validation.py):
  ...
  - Implements simulated normal vs. stressed Paddy growth in [crop_intelligence/run_experiments.py](file:///Users/ananyalakshmi/PES/AgriTech/code/crop_intelligence/run_experiments.py) (e.g., simulating a 35% NDVI drop and low precipitation during Flowering).
* **Verification Command**:
  ```bash
...
* **Implementation**:
  ...
  - Reduced dimensions using PCA, t-SNE, and UMAP to evaluate spatial clustering in [regional_analysis.py](file:///Users/ananyalakshmi/PES/AgriTech/code/data_engineering/regional_analysis.py).
* **Visual Output**: Saved in `figures/indian_regional_separability_report.png`.
...
* **Implementation**:
  - Implemented [sequence_loader.py](file:///Users/ananyalakshmi/PES/AgriTech/code/temporal_intelligence/sequence_loader.py) with 8,192 sequences split into 80% training and 20% validation.
  - Benchmark Mamba Classifier against LSTM and Transformer models in [temporal_intelligence/run_experiments.py](file:///Users/ananyalakshmi/PES/AgriTech/code/temporal_intelligence/run_experiments.py) under missing dates and cloudy profiles.
* **Verification Command**:
  ```bash
...
* **Implementation**:
  - The crop database [crop_knowledge_db.py](file:///Users/ananyalakshmi/PES/AgriTech/code/crop_intelligence/crop_knowledge_db.py) defines stage sensitivity coefficients ($K_y$ for water stress, $K_t$ for heat stress) matching FAO (Food and Agriculture Organization) paper guidelines:
